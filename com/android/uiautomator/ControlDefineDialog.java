...
public class ControlDefineDialog extends Dialog {
    ...
    public void addControl() {
        ...
        String desp = mDescriptionText.getText();
        String name = mControlNameText.getText();
        
        controlInfoMap.put("desp", desp);
        controlInfoMap.put("controlName", name);
        controlInfoMap.put("class", mControlClass);
        
        if (verifyControlDesc(desp) && verifyControlName(name)) {
    ...
    public boolean verifyControlDesc(String description) {
    	boolean result = true;
    	if(description == null || description.length() == 0)
    	{
    		 warning("元素名称不能为空，请重新输入变量名");
             result = false;
    	}
        
        for (int i = 0; i<controlList.size(); ++i) {
            if (controlList.get(i).get("desp").equals(description)) {
                warning("已存在相同的元素名称，请重新输入元素名称");
                result = false;
            }
        }
        return result;
...
//                writer.write("    @Xpath(xpath={\"" + commonxpath + "\"})\r\n");
//                writer.write("    @Description(description=\"" + description + "\")\r\n");
                ...
                writer.write("    public static final String " + name + "_name =\""+ description +"\";\r\n");
